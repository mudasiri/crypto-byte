...
export const fetchCryptos = createAsyncThunk('cryptos/fetchCryptos', async () => {
  const response = await fetch('https://api.coinlayer.com/live?access_key=26c350c08732798f37b02a134b20b7d2&expand=1&target=GHS&symbols=BTC,ETC,DASH,SMART,TESLA,ADA');
...
export const fetchSingleCryptos = createAsyncThunk('cryptos/fetchSingleCryptos', async (id) => {
  const response = await fetch(`https://api.coinlayer.com/live?access_key=26c350c08732798f37b02a134b20b7d2&expand=1&target=GHS&symbols=${id}`);
  const data = await response.json();
  return data;
});
